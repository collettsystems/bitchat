...
class AppleBluetoothMeshService: NSObject, MeshServiceProtocol {
    ...
    private func updateAdvertisement() {
        ...
        advertisementData = [
            CBAdvertisementDataServiceUUIDsKey: [AppleBluetoothMeshService.serviceUUID],
    ...
    func startScanning() {
        ...
        centralManager?.scanForPeripherals(
            withServices: [AppleBluetoothMeshService.serviceUUID],
            options: scanOptions
        )
    ...
    private func setupPeripheral() {
        let characteristic = CBMutableCharacteristic(
            type: AppleBluetoothMeshService.characteristicUUID,
            properties: [.read, .write, .writeWithoutResponse, .notify],
            value: nil,
            permissions: [.readable, .writeable]
        )
        
        let service = CBMutableService(type: AppleBluetoothMeshService.serviceUUID, primary: true)
    ...
    private func sendAnnouncementToPeer(_ peerID: String) {
        ...
        if let data = packet.toBinaryData() {
            ...
            if let peripheral = connectedPeripherals[peerID],
               peripheral.state == .connected,
               let characteristic = peripheral.services?.first(where: { $0.uuid == AppleBluetoothMeshService.serviceUUID })?.characteristics?.first(where: { $0.uuid == AppleBluetoothMeshService.characteristicUUID }) {
...
extension AppleBluetoothMeshService: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        peripheral.delegate = self
        peripheral.discoverServices([AppleBluetoothMeshService.serviceUUID])
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if centralManager?.state == .poweredOn {
            ...
            centralManager?.stopScan()
            centralManager?.scanForPeripherals(withServices: [AppleBluetoothMeshService.serviceUUID], options: [CBCentralManagerScanOptionAllowDuplicatesKey: false])
        }
    }
}

extension AppleBluetoothMeshService: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in services {
            peripheral.discoverCharacteristics([AppleBluetoothMeshService.characteristicUUID], for: service)
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in characteristics {
            if characteristic.uuid == AppleBluetoothMeshService.characteristicUUID {
                ...
                if let vm = self.delegate as? ChatViewModel {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self, weak peripheral] in
                        guard let self = self,
                              ...
                              peripheral.state == .connected,
                              let characteristic = peripheral.services?.first(where: { $0.uuid == AppleBluetoothMeshService.serviceUUID })?.characteristics?.first(where: { $0.uuid == AppleBluetoothMeshService.characteristicUUID }) else { return }
...
extension AppleBluetoothMeshService: CBPeripheralManagerDelegate {
